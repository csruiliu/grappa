...
namespace Grappa{
    ...
    template< GlobalCompletionEvent * GCE = &impl::local_gce, int64_t Threshold = impl::USE_LOOP_THRESHOLD_FLAG, typename T = decltype(nullptr), typename V = decltype(nullptr),typename F = decltype(nullptr) > void forall(GlobalAddress<HashMap<T,V>> self, F visit) {forall<GCE,Threshold>(self->begin(), self->ncells(), [visit](typename HashMap<T,V>::Cell& c){
	for (auto& e : c.entries) {
	    visit(e.key, e.val);
	}
    });
